...
приложениях имеющих веб-интерфейс и бизнес-логикой реализованной на удаленном сервер. В связи с этим полноценное
...
| `kkm.documentClosedChecking.attempts` | Число попыток проверить корректность закрытия документа                                               |
| `kkm.documentClosedChecking.waiting`  | Пауза между попытками проверить корректность закрытия документа                                       |
...
интерфейс подключения ККМ. В данный момент реализован подключения только по COM-порту (в том числе виртуальному, т.е.
подключение через USB). Аргументы для данного типа подключения:
...
Больше примеров запросов к API можно найти в директории `.\php\`. Представленный в этой директории код не является
...
- Библиотека из состава АТОЛ Драйвер ККТ v10.10.6+ ([Оф.сайт](https://fs.atol.ru/))
...
- АТОЛ Драйвер ККТ версии не ниже 10.10.6 ([Оф.сайт](https://fs.atol.ru/))
...
| `install_deps.cmd`  | Установка зависимостей и копирование необходимых заголовочных файлов из `%programfiles%\ATOL\Drivers10\KKT\langs\cpp\fptr10` в `.\deps\fptr10` |
| `test.cmd`          | Сборка отладочной версии и запуск unit-тестов                                                                                                  |
| `build_mprof.cmd`   | Сборка отладочной версии с включенным профилированием памяти                                                                                   |
...
После сборки одним из скриптов `build_*.cmd` в директорию `.\_build` будут установлены файлы `kkmha.exe`,
`libcrypto-X-x64.dll`, `libssl-X-x64.dll`. Так же в этой директории уже находятся файлы конфигурации и т.п., формирующие
готовое окружение для запуска.
